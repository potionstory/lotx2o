...
router.post('/save', (req, res) => {
    ...
    function savePromise() {
        ...
        .then((results) => {
            Account.findOneAndUpdate({ username: req.body.username }, { $inc: { money: -money }}, (err, account) => {
...
router.post('/remove', (req, res) => {
    Lotto.remove({ _id: req.body.id }, (err, lotto) => {
        if (err) throw err;
        Account.findOneAndUpdate({ username: req.session.loginInfo.username }, { $inc: { money: +1000 }},(err, account) => {
            Lotto.find({ username: account._id })
            .sort({ created: 1 })
            .select('number')
            .exec(function(error, lottos) {
