...
import { lottoNumberUpdate } from 'actions/lotto';
...
class LottoNo extends React.Component {
    ...
    handleSaveButton(value, key) {
        switch(key){
            case 0:
                value < this.props.onNumber[key + 1] ? this.props.onReady(true) : this.props.onReady(false);
                break;
            case 5:
                value > this.props.onNumber[key - 1] ? this.props.onReady(true) : this.props.onReady(false);
                break;
            default:
                value > this.props.onNumber[key - 1] && value < this.props.onNumber[key + 1] ? this.props.onReady(true) : this.props.onReady(false);
        }
    }
        ...
        return (
            <ol className="grey lighten-3">
                {this.props.onNumber.map((n, i) => {
                    return (
                        <li key={i}><input onChange={this.handleChange} onBlur={this.handleBlur} type="number" name={i} className={array[i] + ' lighten-2'} value={n} placeholder="?"/></li>
                    )
                })}
            </ol>
        );
    ...
    loopUserNumber() {
        ...
        for(let i = 0; i < 6; i++) {
            ...
            let color = this.state.color[digit];
            ((i) => {
                this.props.onUserNumber[i] == '' ? array[i] = 'grey' : array[i] = color;
            })(i);
        }
        ...
        return (
            <ol className="grey lighten-3">
                {this.props.onUserNumber.map((n, i) => {
                    return (
                        <li key={i}><span className={array[i] + ' lighten-2'}>{n}</span></li>
                    )
                })}
            </ol>
        );
    ...
    render () {
        const buttonList = (
            <span className="btns">
                <a onClick={this.handleRefresh} className="btn-floating waves-effect waves-light blue"><i className="material-icons">refresh</i></a>
                {this.props.onIndex != 0 ? <a onClick={this.handleDelete} className="btn-floating waves-effect waves-light red darken-2"><i className="material-icons">clear</i></a> : ''}
            </span>
        );
        const buttonUser = (
            <span className="btns">
                <a onClick={this.handleRemove} className="btn-floating waves-effect waves-light red darken-2"><i className="material-icons">clear</i></a>
            </span>
        );
        return (
            <li className="list-number">
                ...
                <div>
                    {this.props.onCreateNumber ? this.loopNumber() : this.loopUserNumber()}
                </div>
